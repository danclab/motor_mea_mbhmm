...
% Fit the model
system(sprintf('Rscript ../../R/hmm/fit_plnorm_tv_covar_new.R "%s"',...
     strrep(output_path,'\','/')));
...
run_perm_test_events_timings(data, model, dates)
run_perm_test_events_activations(data, model)
run_perm_test_events_durations(data, model)
run_perm_test_events_days(subject, data, model)

system(sprintf('Rscript ../../R/hmm/shuffle_electrode_plnorm_tv_covar_new.R "%s" %d 1',...
...
system(sprintf('Rscript ../../R/hmm/shuffle_temp_plnorm_tv_covar_new.R "%s" %d 1',...
    strrep(output_path,'\','/'), model.n_states));
...
system(sprintf('Rscript ../../R/hmm/analyze_covars.R "%s" %s',...

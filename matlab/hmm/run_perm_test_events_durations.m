function run_perm_test_events_durations(data, model)
...
for i=1:model.n_states
    ...
    for e=1:length(events)
        ...
        for j=1:model.n_states
            if j~=i
                jdur=trial_state_durations{j};
                if j>1
                    other_event_times(end+1,:)=jdur;
                end                
            end
        end
        
        b_dur=mdur;
        act_diff=partialcorr(b_dur',e_times',other_event_times',...
            'rows','complete','type','Spearman');
        shuffled_diffs=[];
        for j=1:n_perms
            p_idx=randperm(length(e_times));
            shuffled_e_times=e_times(p_idx);
            shuffled_diffs(j)=partialcorr(b_dur',shuffled_e_times',...
                other_event_times(:,:)', 'rows','complete','type','Spearman');
        end        
        b=sum(abs(shuffled_diffs)>=abs(act_diff));
        p = (b+1)/(length(shuffled_diffs)+1);
        disp(sprintf('State: %d, Event: %s, Duration, r=%.3f, p=%.3f', i, event, act_diff, p));

function plotHMM_aligned_elec_shuffled(model, data, varargin)
...
for shuf_idx=1:params.n_shuffs
    forward_probs_fname=sprintf('forward_probs_tv_%s_elec_shuf_%d.csv',model.name, shuf_idx);
    fp=readtable(fullfile(model.path, forward_probs_fname));
    shuf_forward_probs=[];
        ...
        for t_idx=1:data.ntrials
            ...
            if length(trial_rows)
                ...
                for i=1:model.n_states
                    ...
                    sprobs=fp.(sprintf('fw_prob_S%d',state_idx));
                    shuf_forward_probs(t_idx,i,r,1:length(event_wdw)) = sprobs(trial_rows(event_wdw));
                end
                
            end
        end
    end
    forward_probs(shuf_idx,:,:,:)=mean(shuf_forward_probs);
end
...
for r=1:length(align_events)
    ...
    for m=1:length(data.electrodes)
        ...
        StdDev_fr=squeeze(std(firing_rates(:,m,r,:)));
...
for r=1:length(align_events)
    ...
    for m=1:model.n_states
        ...
        StdDev_pstate=squeeze(nanstd(forward_probs(:,m,r,:)));
